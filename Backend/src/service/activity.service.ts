...
import { Activity, ActivityStatus, ActivityType } from '../entity/activity.entity';
...
export class ActivityService {
  ...
  async createActivity(data: CreateActivityDTO) {
    try {
      // 验证创建者是否存在且有权限
      const creator = await this.userModel.findOne({ where: { id: data.creator_id } });
      if (!creator) {
      ...
      // 验证场馆是否存在且可用
      const venue = await this.venueModel.findOne({ where: { id: data.venue_id } });
      if (!venue) {
  ...
  async getActivities(params: {
    ...
    const { status, type, venue_id, creator_id, page = 1, pageSize = 10 } = params;
    
    const queryBuilder = this.activityModel.createQueryBuilder('activity')
    ...
    if (creator_id) {
      queryBuilder.andWhere('activity.creator_id = :creator_id', { creator_id });
    }
  ...
  // 更新活动
  async updateActivity(activityId: number, data: UpdateActivityDTO, operatorId: number) {
    try {
      const activity = await this.activityModel.findOne({ 
        where: { id: activityId },
        relations: ['creator']
      ...
      if (activity.creator_id !== operatorId && operator.role !== 'super_admin') {
        return { code: 3, message: '权限不足，只有创建者或超级管理员可以修改活动' };
      }
      ...
      const registration_deadline = data.registration_deadline || activity.registration_deadline;
  ...
  // 更新活动状态
  async updateActivityStatus(activityId: number, status: ActivityStatus, operatorId: number) {
    try {
      const activity = await this.activityModel.findOne({ 
        where: { id: activityId },
        relations: ['creator']
  ...
  async deleteActivity(activityId: number, operatorId: number) {
    return this.updateActivityStatus(activityId, ActivityStatus.CANCELLED, operatorId);
  }
